...
const createGroupChat = TryCatch(async (req, res, next) => {
  ...
  if (members.length < 2)
    return next(new ErrorHandler("Group have atleast 3 Members", 400));
...
const addMembers = TryCatch(async (req, res, next) => {
  ...
  if (chat.creator.toString() !== req.user.toString())
    return next(new ErrorHandler("Your Not Allowed to Add Members", 403));
...
const removeMember = TryCatch(async (req, res, next) => {
  ...
  if (chat.members.length <= 3)
    return next(new ErrorHandler("Group must have Atleast Members", 400));
  ...
  emitEvent(
    ...
    ALERT,
    chat.members,
    `${userThatWillBeRemoved.name} has been Removes from Group`
  );
...
const leaveGroup = TryCatch(async (req, res, next) => {
  ...
  emitEvent(req, ALERT, chat.members, `${user.name} has been left the Group`);
...
const sendAttachments = TryCatch(async (req, res, next) => {
  ...
  if (files.length < 1)
    return next(new ErrorHandler("Attachments not Found", 400));
